        ...
        ${entries.map((entry, index) => `
          <article class="diary-card" data-diary-entry-index="${index}" role="button" tabindex="0">
            ...
            <div class="tag-row" style="margin-top:10px">
              ...
              <span class="meta">연결: ${diaryHasLinkedRecord(entry.linked) ? entry.linked : "관련 기록 없음"}</span>
            </div>
          </article>
        `).join("")}
      </div>
...
// Restore record detail inner flow: linked diaries and edit always route to their page screens.
function openMemoryDetailLatestV3(index, backAction = null) {
  const safeIndex = Number.isFinite(Number(index)) ? Number(index) : 0;
  state.activeMemoryIndex = safeIndex;
  const memory = state.memories[safeIndex] || state.memories[0];
  ...
  const sheet = qs(".modal-sheet");
  qs("[data-memory-detail-back]").addEventListener("click", () => runFlowBack(goBack));
  qsa("[data-linked-diary-index]", sheet).forEach((card) => {
    card.addEventListener("click", () => openLinkedDiaryDetailLatest(Number(card.dataset.linkedDiaryIndex), () => openMemoryDetailLatestV3(safeIndex, backAction)));
  });
  qs("[data-memory-edit-page]").addEventListener("click", () => {
    openMemoryEditPageLatest(safeIndex, () => openMemoryDetailLatestV3(safeIndex, backAction));
  });
...
if (!window.__duariRecordDetailInnerFlowRestore) {
  window.__duariRecordDetailInnerFlowRestore = true;
  window.addEventListener("click", (event) => {
    const linkedDiary = event.target.closest?.("#modal [data-linked-diary-index]");
    if (linkedDiary) {
      event.preventDefault();
      event.stopPropagation();
      event.stopImmediatePropagation();
      const memoryIndex = typeof state.activeMemoryIndex === "number" ? state.activeMemoryIndex : 0;
      openLinkedDiaryDetailLatest(Number(linkedDiary.dataset.linkedDiaryIndex), () => openMemoryDetailLatestV3(memoryIndex));
      return;
    }

    const editButton = event.target.closest?.("#modal [data-memory-edit-page]");
    if (editButton) {
      event.preventDefault();
      event.stopPropagation();
      event.stopImmediatePropagation();
      const memoryIndex = Number(editButton.dataset.index || state.activeMemoryIndex || 0);
      openMemoryEditPageLatest(Number.isFinite(memoryIndex) ? memoryIndex : 0, () => openMemoryDetailLatestV3(Number.isFinite(memoryIndex) ? memoryIndex : 0));
    }
  }, true);
}
